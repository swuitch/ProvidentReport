...
namespace ProvidentReport.Repositories
{
    public class DeliveryReportRepository
    {
                ...
                try
                {
                    ...
                    if (batchno.Contains("ER"))
                    {
                        var output = db.Query<Employer>("select a.area_code,c.description,a.status_code, " +
                                                        ...
                                                        "where  a.batchno=@batchno", new {batchno}).ToList();

                        foreach (var item in output.OrderBy(a => a.status_code).ToList())
                        {
                            var abc = data.ToList();
                            var c =
                                data.Where(b => b.branch_name == item.branch_name && b.area_code == item.area_code)
                                    .SingleOrDefault();
                            if (c == null)
                            {
                    ...
                    else
                    {
                        var output =
                            db.Query<Employer>(
                                "select a.delivery_area,a.status_code,a.trackno,a.branch_name,a.cutdate,a.unit_price,a.penalty,sum(a.days_delay) as days_delay,count(*) as num,a.hub_name from " +
                                ...
                                " group by a.trackno,a.branch_name,a.cutdate,a.unit_price,a.status_code,a.penalty,a.hub_name,a.delivery_area ",
                                new {batchno}).ToList();
            ...
            using (var db = new IngresConnection(ConfigurationManager.ConnectionStrings["pfmdb"].ConnectionString))
            {
                ...
                try
                {
                    ...
                    if (batchno.Contains("ER"))
                    {
                        data =
                            db.Query<Employer>(
                                "select distinct a.pagibig_erid,a.batchno,a.pickdate,a.cutdate,a.pod_date,a.days_delay,a.unit_price,a.penalty, " +
                    ...
                    else
                    {
                        data =
                            db.Query<Employer>(
                                "select distinct a.pagibigid,a.batchno,a.pickdate,a.cutdate,a.pod_date,a.days_delay,a.unit_price,a.penalty, " +
                                " b.description as delivery_status, " +
                                " c.description as delivery_area, " +
        ...
        public static int UpdateEmployer(List<Employer> ds)
        {
            using (var db = new IngresConnection(ConfigurationManager.ConnectionStrings["pfmdb"].ConnectionString))
            {
                ...
                var employer = "update lo_stl_billing_employer " +
                ...
                var area = db.Query<Area>("select * from lo_stl_billing_delivery_area").ToList();
                ...
                var result = db.Query<Employer>("select sum(num_envelope) as num from lo_stl_billing_employer " +
                                                "where batchno=@batchno", new {batchno}).SingleOrDefault();
